...
public class AuthController {
	...
	public ResponseEntity<?> refresh(HttpServletRequest request, HttpServletResponse response) {
		...
		Member member = memberRepository.findByEmail(username).orElseThrow(() -> new RuntimeException("사용자 없음"));
		...
		Authentication authentication = new UsernamePasswordAuthenticationToken(username, null,
				member.getMemberRoleSet().stream().map(role -> new SimpleGrantedAuthority("ROLE_" + role.name()))
	...
	public ResponseEntity<?> logout(HttpServletResponse response,Authentication authentication) {
	    ...
	    if (authentication != null) { // 로그인 상태인지 확인
	        String username = authentication.getName(); // JwtCheckFilter에서 세팅된 이메일
